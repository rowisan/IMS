 ...
 * @author ROSHAN
 */
public class PaymentOnAccountController extends Action {
    ...
    protected void save() {
        try {
            CommonLogger.log("DEBUG : ", PaymentOnAccountController.class.getName(), "save() >>");
            ...
            String payTerm = "";
            String type = VoucherType.VOUCHER_ON_ACCOUNT;
            ...
            paymentMethodVDO.setPaymentSetting(PaymentSetting.PAYMENT_ON_ACCOUNT_CASH);
            ...
            CommonLogger.log("DEBUG : ", PaymentOnAccountController.class.getName(), "save() >> " + msg);
            request.setAttribute(SessionVariables.MSG, msg);
            RequestDispatcher dispatcher = request.getRequestDispatcher("/PaymentOnAccountServlet?FUNC=" + FormAction.SAVE);
            dispatcher.forward(request, response);
        } catch (ServletException ex) {
            CommonLogger.handle(ex, PaymentOnAccountController.class.getName(), "save()");
            Logger.getLogger(PaymentOnAccountController.class.getName()).log(Level.SEVERE, null, ex);
    ...
    protected void search() {
        try {
            RequestDispatcher dispatcher = request.getRequestDispatcher("/PaymentOnAccountServlet?FUNC=" + FormAction.SEARCH);
            dispatcher.forward(request, response);
        } catch (ServletException ex) {
            CommonLogger.handle(ex, PaymentOnAccountController.class.getName(), "save()");
            Logger.getLogger(PaymentOnAccountController.class.getName()).log(Level.SEVERE, null, ex);
