 ...
 * @author ROSHAN
 */
public class ReceiptAgPackageController extends Action {
    ...
    protected void save() {
        try {
            CommonLogger.log("DEBUG : ", ReceiptAgPackageController.class.getName(), "save() >>");
            int cpu = Integer.parseInt(request.getParameter("txtPackage"));
            String customer = request.getParameter("txtCustCodeH");
            String customerCode = request.getParameter("txtCustCode");
            String customerName = (String) request.getParameter("txtName");
            ...
            String payTerm = "";
            String type = VoucherType.RECEIPT_AG_PACKAGE;
            ...
            voucherVDO.setCustomer(customer);
            voucherVDO.setCustomerCode(customerCode);
            voucherVDO.setCustomerName(customerName);
            voucherVDO.setCpu(String.valueOf(cpu));
            voucherVDO.setVoucherDate(voucherDate);
            voucherVDO.setDocumentNo(docunetNo);
            voucherVDO.setRemark(remark);
            ...
            paymentMethodVDO.setPaymentSetting(PaymentSetting.RECEIPT_AG_PACKAGE_CASH);
            ...
            CommonLogger.log("DEBUG : ", ReceiptAgPackageController.class.getName(), "save() >> " + msg);
            request.setAttribute(SessionVariables.MSG, msg);
            RequestDispatcher dispatcher = request.getRequestDispatcher("/ReceiptAgPackageServlet?FUNC=" + FormAction.SAVE);
            dispatcher.forward(request, response);
        } catch (ServletException ex) {
            CommonLogger.handle(ex, ReceiptAgPackageController.class.getName(), "save()");
            Logger.getLogger(ReceiptAgPackageController.class.getName()).log(Level.SEVERE, null, ex);
    ...
    protected void search() {
        try {
            RequestDispatcher dispatcher = request.getRequestDispatcher("/ReceiptAgPackageServlet?FUNC=" + FormAction.SEARCH);
            dispatcher.forward(request, response);
        } catch (ServletException ex) {
            CommonLogger.handle(ex, ReceiptAgPackageController.class.getName(), "save()");
            Logger.getLogger(ReceiptAgPackageController.class.getName()).log(Level.SEVERE, null, ex);
